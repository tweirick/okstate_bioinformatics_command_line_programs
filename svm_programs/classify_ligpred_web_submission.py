...
if isvalidatefasta(fasta_seqs):# Validate format of received fasta data.     
    ...
    for predictor in sorted(predictor_list):
        ...
        for model_file_name in sorted_model_file_glob:
            ...
            #writerefreshpage(saved_data_url,bookmark_link+"<br/>"+"<br/>classification of "+class_name+
            #                 " complete.<br/>"+"\n".join(progress_list)+
            #                 "<br/>"+"<br/>".join(model_progress))
        ...
        log_file_list.append("Classification of "+predictor+" complete. "+ctime(time())+" ("+str(time()-classif_t1)+" seconds)")
        writerefreshpage(saved_data_url,bookmark_link+d1+"".join(model_progress)+d2)
