...
c_run_conditions = []
for i in range(0,501,60):
    c_run_conditions.append(i)

g_run_conditions = []
for i in range(1,1000,200):
    g_run_conditions.append(i)

#1-15
j_run_conditions = []
for i in range(1,15,2):
    j_run_conditions.append(i)

#d run 1-15
d_run_conditions = []
for i in range(0,15,2):
    d_run_conditions.append(i)
...
def run_svm_learn_RDF(pos_neg_to_five_fold_dict,
                             c_run_conditions,d_run_conditions,g_run_conditions):
    ...
    for pos_neg_file in pos_neg_to_five_fold_dict:    
        ...
        four_name_list = []
        for g_cond in g_run_conditions:
            for d_cond in d_run_conditions:
                for c_cond in c_run_conditions:
                    c_cond = str(c_cond)
                    d_cond = str(d_cond)
                    g_cond = str(g_cond)
                    output_file_name = (pos_neg_file+
                            "_c"+c_cond+"_d"+d_cond+"_g"+g_cond+".lrn")
                    
                    print(output_file_name)
                    
                    subprocess_string = ("./svm_learn -z c -t 2"+" -c "+c_cond+" "+" -j "+d_cond+" "+" -g "+g_cond+" "+pos_neg_file+" "+output_file_name)
                    ...
                    five_fold_name = pos_neg_to_five_fold_dict[pos_neg_file]
                    #
                    parent_and_condition_name = five_fold_name+"_c"+c_cond+"_d"+d_cond+"_g"+g_cond
...
def addfivefoldresults(run_conditions,output_results_name):        
    ...
    for condition in run_conditions:
        ...
        print("Count file name:",five_fold_glob,"*_"+str(condition).replace(".","")+'*.cnt',"*_"+str(condition)+'*.cnt')
...
def poly_addfivefoldresults(c_run_conditions,d_run_conditions,output_results_name):        
    '''
    This function reads the individual run outputs and 
    '''
    #_50lrnscr.matrix.cnt
    ...
    for d_run_cond in d_run_conditions:
    
        for c_run_cond in c_run_conditions:
            
            five_fold_glob = glob( "*_c"+str(c_run_cond).replace(".","")+"_d"+str(d_run_cond).replace(".","")+"lrnscr.matrix.cnt" )
    
            #print("Count file name:",five_fold_glob, "*_c"+str(c_run_cond).replace(".","")+"_d"+str(d_run_cond).replace(".","")+"lrnscr.matrix.cnt")
            average_calcs_dict = {"TOTAL":0}
            ...
            out_str = ["aac_"+str(c_run_cond)+","+"aad_"+str(d_run_cond)]
            for calc in sorted(list(average_calcs_dict.keys())):
...
def RDF_addfivefoldresults(c_run_conditions,d_run_conditions,g_run_conditions,output_results_name):        
    '''
    This function reads the individual run outputs and 
    '''
    #_50lrnscr.matrix.cnt
    zzzzz_out_numbers_list = []
    for g_run_cond in g_run_conditions:
        for d_run_cond in d_run_conditions:
            for c_run_cond in c_run_conditions:
                five_fold_glob = glob( "*_c"+str(c_run_cond).replace(".","")+
                                       "_d"+str(d_run_cond).replace(".","")+
                                       "_g"+str(g_run_cond).replace(".","")+
                                       "lrnscr.matrix.cnt" )
                ...
                out_str = ["c_"+str(c_run_cond)+","+"d_"+str(d_run_cond)+","+"g_"+str(g_run_cond)]
                for calc in sorted(list(average_calcs_dict.keys())):
...
vartmp = run_svm_learn_RDF(pos_neg_to_five_fold_dict,
                                  c_run_conditions,
                                  j_run_conditions,
                                  g_run_conditions)
...
print("Elements of lrn_to_pos_neg_vec_file_dict")
for d_el in lrn_to_pos_neg_vec_file_dict:
    print("Key=",d_el,"value=",lrn_to_pos_neg_vec_file_dict[d_el])

print("Elements of five_fold_and_name_dict_to_members_list")
for d_el in five_fold_and_name_dict_to_members_list:
    print("Key=",d_el,"value=",five_fold_and_name_dict_to_members_list[d_el])
...
RDF_addfivefoldresults(c_run_conditions,
                       j_run_conditions,
                       g_run_conditions,output_results_name)        

#poly_addfivefoldresults(c_run_conditions,d_run_conditions,output_results_name)
