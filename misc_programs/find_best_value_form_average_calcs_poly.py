...
for file_name in file_glob:
    ...
    for line in open(file_name,'r'):
        ...
        c_condition,d_condition = split_line[-1].split(",")
        ...
        list_rank.append([float(c_condition),float(d_condition),mcc_value])
        ...
        if largest_value == None:
            ...
            largest_value  = mcc_value
            best_condition = c_condition,d_condition


    out_to_csv_file_list = []
    print("cvalue,dvalue,mcc")
    
    for e in sorted(list_rank):#,reverse=True
        tmp_out_str = str(e[0])+","+str(e[1])+","+str(float(e[2])*100)[0:4]
        print(tmp_out_str)   
    ...
    out_file = open(file_name+".best"+
                    "_c"+str(best_condition[0])+
                    "_d"+str(best_condition[1])+".csv",'w')
