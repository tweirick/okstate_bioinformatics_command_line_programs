...
def maketrainingandtestingsets(class_vecs_dict,run_params_dict,fold_level):
    ...
    for vec_class in class_vecs_dict:
        ...
        print(vec_class,len(class_vecs_dict[vec_class]),len(tmp_training_vecs),len(tmp_test_vecs),start,stop)
...
def oneagainstrestkfoldtest(
    ...
    print(z_class_folds_ls_d['4CL_pth4.faa.0.9.faa.AACOMP.SVM_LIGHT.vec'][0])

    class_name_list = sorted(list(p_class_folds_ls_d.keys()))
    
    svm_kernal="rbf"
    overall_result_dict = {"FP":0,"FN":0,"TN":0,"TP":0}
    n = len(class_name_list)

    overall_confusion_matrix = {
        y:{x:0 for x in class_name_list} for y in class_name_list
    }

    class_performance = {}
    for key_el in class_name_list:
    ...
    for fold_level in range(0,n_folds):
        ...
        class_perf_out_list.append("Fold: "+str(fold_level))
        ...
        #For each class 
        for class_of_vecs_being_tested in class_name_list:
            pred_values_list = []
            #fold x class1 ...

            #Make model, look through, get neg for all not equal to selected class.  
            training_vector = []
            for vec_class_name in class_name_list:
                #Get class training and testing sequences for a fold. 
                #num_of_vecs_in_class = len(class_vecs_dict[vec_class])

                #Add all but 
                for i in range(0,run_params_dict["max_folds"]):
                    if i != fold_level:
                        if vec_class_name == class_of_vecs_being_tested:
                            #Get positive values.
                            training_vector+=p_class_folds_ls_d[vec_class_name][i]
                        else:
                            #Get Negative values.
                            training_vector+=n_class_folds_ls_d[vec_class_name][i]
            ...
            for test_vector_class in class_name_list:
                #For each training set classify the vectors. 
                
                #Change this to do whole classification at once. 
                print(test_vector_class)
                print(len(z_class_folds_ls_d[test_vector_class]))
                print(z_class_folds_ls_d['4CL_pth4.faa.0.9.faa.AACOMP.SVM_LIGHT.vec'][0])

                test_vecs_list = z_class_folds_ls_d[test_vector_class][fold_level]

                print(test_vector_class,fold_level)
                print(test_vecs_list)
                print(len(test_vecs_list))
                pred_values_dict = {}
...
def makepysvmlightvectorlist(file_name_list,input_file_format):
    ...
    pos_vector_list_dict  = {}
    zero_vector_list_dict = {}
    neg_vector_list_dict  = {}
    ...
    for file_name in file_name_list: 
        ...
        for example_vector_list in open(file_name,"r"):
            if input_file_format == "SVMLIGHT":
                ...
                p_vector_list.append( (1,tmp_list) )
                n_vector_list.append( (-1,tmp_list) )
                z_vector_list.append( (0,tmp_list) )
            else:
                print("ERROR")
        #Add a vector class to the classification dict
        #vector_list_dict.update( {class_name:vector_list} )
        pos_vector_list_dict.update(  {class_name:p_vector_list} )
        zero_vector_list_dict.update( {class_name:n_vector_list} )
        neg_vector_list_dict.update(  {class_name:z_vector_list} )

    return pos_vector_list_dict,zero_vector_list_dict,neg_vector_list_dict


def makefolds(dict_of_class_lists,n_folds):
    """list val of dict -> n_fold el list with 1/n_fold prev list els in each. 
    """

    class_folds_lists_dict = {}


    """
    Make a list with 1/n_fold els in list 
    """
    for vec_class in dict_of_class_lists:

        tmp_class_vec_folds = []

        min_els_per_fold = len(dict_of_class_lists[vec_class])/int(n_folds)
        remainder        = min_els_per_fold % n_folds
        ...
        for fold_level in range(0,n_folds):
            ...
            if fold_level == 0:
                #Fold 0
                tmp_fold_vecs = dict_of_class_lists[vec_class][stop:]
            elif fold_level == n_folds-1:
                #Last Fold
                tmp_fold_vecs = dict_of_class_lists[vec_class][:start-1]
            else:
                #Middle Fold
                tmp_fold_vecs = dict_of_class_lists[vec_class][:start]+dict_of_class_lists[vec_class][stop:]
            
            #print()
            tmp_class_vec_folds.append(tmp_fold_vecs)
            print(vec_class,
                  len(dict_of_class_lists[vec_class]),
                  len(tmp_fold_vecs),start,stop)
        
        class_folds_lists_dict.update({vec_class:tmp_class_vec_folds})

        
    return class_folds_lists_dict

def makemodels(class_folds_lists_dict):

    return class_test_sets_
...
Ex: 
{"class1":[ (0, [(1, 0.13456), (2, 0.02902), (3, 0.04749),...]),...},
{"class2":[(0, [(1, 0.08333), (2, 0.03916), (3, 0.24749),...]),...},...
"""
#dict_of_class_lists = makepysvmlightvectorlist(file_name_glob,input_file_format)
p_class_vec_l_d,z_class_vec_l_d,n_class_vec_l_d = makepysvmlightvectorlist(
                                                  file_name_glob,
                                                  input_file_format)

#print(p_class_vec_l_d.keys())
#print(z_class_vec_l_d.keys())
#print(n_class_vec_l_d.keys())
"""
Split the data into folds. So each key will now point to a list n_fold
elements long. Each containing ~1/n_fold vectors
Ex: 
{"class1":[(1, 0.13456), (2, 0.02902), (3, 0.04749),...],
          [(1, 0.79456), (2, 0.01502), (3, 0.08739)],...},
{"class1":[(1, 0.13456), (2, 0.02902), (3, 0.04749),...],
          [(1, 0.79456), (2, 0.01502), (3, 0.08739)],...},
"""
p_class_folds_ls_d = makefolds(p_class_vec_l_d,n_folds)
z_class_folds_ls_d = makefolds(z_class_vec_l_d,n_folds)
n_class_folds_ls_d = makefolds(n_class_vec_l_d,n_folds)

#print(p_class_folds_ls_d.keys())
#print(z_class_folds_ls_d['4CL_pth4.faa.0.9.faa.AACOMP.SVM_LIGHT.vec'][0])
#print(n_class_folds_ls_d.keys())

#print(len(p_class_folds_ls_d[p_class_folds_ls_d.keys()[1]]))
#print(len(z_class_folds_ls_d[z_class_folds_ls_d.keys()[1]]))
#print(len(n_class_folds_ls_d[n_class_folds_ls_d.keys()[1]]))
#"""
#Make all models and test sets.
#"""
...
for c_val in range(c_start,c_end,c_parts):
    for c_ratio_val in range(j_start,j_end,j_parts):#j
        for gamma_val in range(g_start,g_end,g_parts):
            ...
            overall_perf_dict,class_perf_str = oneagainstrestkfoldtest(
                p_class_folds_ls_d=p_class_folds_ls_d,
                z_class_folds_ls_d=z_class_folds_ls_d,
                n_class_folds_ls_d=n_class_folds_ls_d,
