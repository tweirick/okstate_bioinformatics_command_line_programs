...
def dipedtidecomposition(fasta_entry):
    ...
    for i in range(0,fasta_entry_len-1):
        dipep_seq = fasta_entry[i:i+2]
        assert len(dipep_seq) == 2
        if dipep_seq in dipeptide_dict:
            dipeptide_dict[dipep_seq]+=1
...
for file_name in file_glob:
    ...
    output_file_name = file_name+".dipep1vec" 
